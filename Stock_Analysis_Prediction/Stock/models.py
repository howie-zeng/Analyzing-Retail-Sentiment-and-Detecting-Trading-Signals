...
class StockPredictor:
    ...
    def fit_predict(self):
        ...
        for start in tqdm(range(0, self.X.shape[0] - self.window_size), desc="Processing"):
            ...
            current_prices.append(X_test['Close'])
    ...
    def plot_residuals(self):
        residuals_percentage = [(true - pred)/true * 100 for true, pred in zip(self.true_values, self.predictions)]
        data = self.df

        plt.figure(figsize=(12, 6))
        plt.scatter(data.index.values[self.window_size:], residuals_percentage, alpha=0.5)
        plt.axhline(0, color='red', linestyle='--')
        plt.title('Residuals Analysis (in percentage)')
    ...
    def plot_actual_vs_predicted(self, prediction_lag):
        ...
        plt.plot(time_indices, self.current_prices, label='Current Prices', linewidth=2, color='blue')
    ...
    def trading_strategy(self, window_size=5, starting_funds=50000):
        # Trading related computations
        self.df.loc[self.df.index[window_size:], 'predictions'] = self.predictions
        self.df.loc[self.df.index[window_size:], 'prediction_return'] = (self.df['predictions'] - self.df['Close']) / self.df['Close']
        self.df['return_5_days'] = (self.df['Close_5_days_later'] - self.df['Close']) / self.df['Close']

        # Initialize variables
        weekly_returns = self.df['Close'].resample('W').ffill().pct_change()
        funds = starting_funds
        stock_position = 0
        date = self.df.index[window_size:]
        stock_price = self.df['Close'].values[window_size:]
        actual_return = self.df['return_5_days'].values[window_size:]
        actual_return_series = pd.Series(actual_return)
        prediction_return = self.df['prediction_return'].values[window_size:]
        portfolio_value = []
